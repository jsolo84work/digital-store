            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
                {images.map((_, index) => (
                    <button
                        ...
                        className={`w-2.5 h-2.5 rounded-full transition-all duration-200 ${currentIndex === index ? 'bg-pink-600 scale-110' : 'bg-gray-300'
                            }`}
                ...
                <div className="flex justify-center mt-4 gap-2 flex-wrap">
                    {images.map((image, index) => (
                        <img
                            ...
                            className={`w-20 h-16 object-cover border-2 cursor-pointer transition ${currentIndex === index ? 'border-pink-600' : 'border-transparent'
                                } rounded-${radius}`}
                            onClick={() => setCurrentIndex(index)}
